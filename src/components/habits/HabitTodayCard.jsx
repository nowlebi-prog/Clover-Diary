...
export default function HabitTodayCard({ habit, done, streak, onToggle, onEdit }) {
  return (
    ...
    >
      ...
      <p className="mt-1 text-xs text-clover-sub">{done ? "오늘 완료했어요!" : "오늘 아직 안 했어요"}</p>
      <p className="mt-3 text-sm font-bold text-clover-deep">🔥 {streak}일 연속</p>
      <div className="mt-4 flex gap-2">
        <AppButton className="flex-1" variant={done ? "soft" : "primary"} onClick={onToggle}>{done ? "체크 취소" : "완료 체크"}</AppButton>
