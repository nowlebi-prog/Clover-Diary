  ...
  return (
    <div className="rounded-[24px] bg-white/45 p-4">
      ...
      <div className="mt-2 flex justify-center gap-4 text-xs font-bold text-clover-sub">
        ...
        <span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-sky-300" />숙면</span>
      </div>
    </div>
  );
}
...
function MonthlyJournalModal({ data, today, onClose }) {
  ...
  return (
    <Modal title="이달의 일기" onClose={onClose}>
      <div className="grid gap-4">
        <p className="text-sm font-bold text-clover-sub">기분, 날씨, 오늘 요약, 감사일기가 날짜별로 모여요.</p>
...
export default function JournalPage() {
  ...
  }, [todayMood?.id, todayReflection?.id, todayGratitude?.id]);
    ...
    next.moodEntries = [
      {
        ...
        createdAt: todayMood?.createdAt || today,
        updatedAt: today
      },
    ...
    next.reflections = [
      {
        ...
        body: summary.trim(),
        memo: summary.trim(),
        createdAt: todayReflection?.createdAt || today,
        updatedAt: today
      },
    ...
    next.gratitudeEntries = [
      {
        ...
        date: today,
        items: gratitude,
        createdAt: todayGratitude?.createdAt || today,
        updatedAt: today
      },
  ...
  return (
    <>
      <PageHeader eyebrow={today} title="오늘 기록">
        <AppButton variant="soft" onClick={() => setShowMonth(true)}>이달의 일기</AppButton>
        <AppButton onClick={saveTodayRecord}>오늘 기록 저장</AppButton>
      </PageHeader>
        ...
        <div className="grid gap-4">
          <GlassCard>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              ...
              <label className="grid gap-1 text-sm font-bold">숙면 시간<AppInput type="number" min="0" max="24" step="0.5" value={sleepHours} onChange={(event) => setSleepHours(event.target.value)} placeholder="예: 7.5" /></label>
          ...
          <GlassCard>
            ...
            <AppButton className="mt-3" onClick={saveTodayRecord}>기분·날씨·요약·감사일기 저장</AppButton>
          </GlassCard>
        </div>
          ...
          <GlassCard>
            ...
            <div className="grid gap-3">
              <div className="rounded-2xl bg-white/55 p-4">
                <p className="text-xs font-black text-clover-deep">기분 / 숙면 / 날씨</p>
      ...
      <button
        ...
        className="fixed bottom-5 right-5 z-30 rounded-full bg-clover-deep px-5 py-3 text-sm font-black text-white shadow-glass"
      >
        이달의 일기
      </button>
