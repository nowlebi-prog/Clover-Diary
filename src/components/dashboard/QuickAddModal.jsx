      ...
      <div className="grid gap-3">
        ...
        {type === "event" && (
          <>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <label className="grid gap-1 text-sm font-bold">분류<AppSelect value={form.category || "개인"} onChange={(event) => set("category", event.target.value)}>{categories.map((item) => <option key={item} value={item}>{item}</option>)}</AppSelect></label>
              <label className="grid gap-1 text-sm font-bold">우선순위<AppSelect value={form.priority || "normal"} onChange={(event) => set("priority", event.target.value)}><option value="high">높음</option><option value="normal">보통</option><option value="low">낮음</option></AppSelect></label>
            </div>
